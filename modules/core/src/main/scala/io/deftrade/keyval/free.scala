...
protected sealed abstract class FreeValueStore[F[_], Value](val Value: WithId[Value]) {
  ...
  /** */
  def compilerFor(
      valueStore: StoreTypes.Aux[F, WithId, Value] with ValueStore[F, Value]
  ): Cmd ~> valueStore.StreamF =
    new FunctionK[Cmd, valueStore.StreamF] {

      /** TODO: consider splitting up read and write */
      def apply[A](command: Cmd[A]): valueStore.StreamF[A] = command match {
...
protected sealed abstract class FreeKeyValueStore[F[_], Key, Value](val V: WithKey.Aux[Key, Value]) {
  ...
  def compilerFor(
    ...
    new FunctionK[Cmd, kvs.StreamF] {
      ...
      /** TODO: consider splitting up read and write */
      def apply[A](command: Cmd[A]): kvs.StreamF[A] = command match {
